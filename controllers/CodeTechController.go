...
type CodeTechController struct {
	UserStatusController
}

//技术专栏页面
func (c *CodeTechController) TechColumn() {
	article_message := ArticleMessage{}
	article_message_arr := []ArticleMessage{}
	article_lists := []models.Article_bases{}
	utils.DB.Where("class_id=?", 1).Order("id desc").Find(&article_lists)
	...
	article_lists = []models.Article_bases{}
	utils.DB.Where("class_id=?", 1).Order("id desc").Offset((pageindex - 1) * pagesize).Limit(pagesize).Find(&article_lists)
